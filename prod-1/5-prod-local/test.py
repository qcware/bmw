...
for k in range(max_ncar):
    ...
    for state in pool_states:
        constellation2 = constellation.copy()
        constellation2 += [state] 
        npass = problem.test_set.npass_constellation(constellation=constellation2)
        total_npass.append(npass)
    pivot = np.argmax(total_npass)
    ...
    print(problem.test_set.counts[pivot])
